		...
		if (tmp.tab == 0) {
			if (tmp.stab[0] == 0) {
				...
				if (tmp.rank_tab == 0) {
					...
					tmp.el.massLift.setDisplay(true)
					tmp.el.massLift.setHTML(`The weight of object is <b>`+formatMass(tmp.massGain)+`</b>, and you're lifting <b>`+format(BUILDINGS.eff("mass_5").add(1))+"</b> of them. You spend "+formatTime(tmp.liftTime)+"s to lift one time. ("+formatTime(player.ticker)+"s / "+formatTime(tmp.liftTime)+"s)")
					tmp.el.LiftPower.setHTML(`<br>Your lifting power is <b>`+format(player.liftingPower,0)+`</b> [Exp to next level: `+format(player.exp,3)+"/"+format(tmp.liftPower,3)+"]")
					tmp.el.LiftPowerEff.setHTML(`<br><span style="color:green;">Boosts object weight by <b>`+formatMult(tmp.LiftPowerEff,4)+`</b></span>`)
