const RANKS = {
    ...
    desc: {
        rank: {
            ...
            '2': "unlock mass upgrade 2, reduce mass upgrade 1 scaling by 20%, Overcounter boosts itself.",
        ...
        tier: {
            '1': "reduce rank requirements by 20%, unlock Charged Upgrades.",
...
function updateRanksTemp() {
    ...
    tmp.ranks.rank.req = E(10).pow(player.ranks.rank.div(ffp2).scaleEvery('rank',false,[1,1,1,1,rt_fp2,1,ifp]).pow(rooted_fp).div(fp).pow(1.25)).div(1e21).mul(127.5)
    tmp.ranks.rank.bulk = E(0)
    if (player.mass.gte(10/1e21)) tmp.ranks.rank.bulk = player.mass.mul(1e21).div(127.5).max(1).log(10).root(1.25).mul(fp).root(rooted_fp).scaleEvery('rank',true,[1,1,1,1,rt_fp2,1,ifp]).mul(ffp2).add(1).floor();
