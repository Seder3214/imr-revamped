...
function calc(dt) {
    ...
    let liftAmt=BUILDINGS.eff('mass_5').add(1)
    if (tmp.pass<=0 && tmp.inf_time == 0) {
         if (player.ticker.lte(tmp.liftTime)) player.ticker = player.ticker.add(E(1).mul(dt))
         if (player.ticker.gte(tmp.liftTime)) {
            player.exp = player.exp.add(tmp.expGain)
            player.mass = player.mass.add(tmp.massGain.mul(liftAmt))
            player.ticker=E(0)
         }
         let x = E(0)
            x = player.exp.max(1).log(10).pow(1.75).root(player.liftingPower.gte(15)||x.gte(15)?1.035:1).root(player.liftingPower.gte(1e9)||x.gte(1e9)?1.65:1).root((player.liftingPower.gte(1e25)||x.gte(1e25)?1.25:1)).root((player.liftingPower.gte(1e50)||x.gte(1e50)?1.5:1)).root((player.liftingPower.gte(1e100)||x.gte(1e100)?1.425:1)).root((player.liftingPower.gte(1e200)||x.gte(1e200)?1.225:1)).add(1).floor()
...
function save(){
    ...
    if (localStorage.getItem("revampSave") == '') wipe()
    localStorage.setItem("revampSave",str)
    tmp.prevSave = localStorage.getItem("revampSave")
...
function loadGame(start=true, gotNaN=false) {
    if (!gotNaN) tmp.prevSave = localStorage.getItem("revampSave")
    wipe()
    load(tmp.prevSave)
